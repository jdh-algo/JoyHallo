...
def train_stage2_process(cfg: argparse.Namespace) -> None:
    ...
    # loading weight from stage1
    stage1_ckpt_dir = cfg.stage1_ckpt_dir
    net.denoising_unet.load_state_dict(
        torch.load(
    ...
    if accelerator.is_main_process:
        logger.info(f"loaded weight from: {os.path.join(stage1_ckpt_dir, f'denoising_unet.pth')}")
    
    net.reference_unet.load_state_dict(
        torch.load(
    ...
    if accelerator.is_main_process:
        logger.info(f"loaded weight from: {os.path.join(stage1_ckpt_dir, f'reference_unet.pth')}")

    net.face_locator.load_state_dict(
        torch.load(
            os.path.join(stage1_ckpt_dir, f"face_locator.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    if accelerator.is_main_process:
        logger.info(f"loaded weight from: {os.path.join(stage1_ckpt_dir, f'face_locator.pth')}")

    net.imageproj.load_state_dict(
        torch.load(
            os.path.join(stage1_ckpt_dir, f"imageproj.pth"),
            map_location="cpu",
        ),
        strict=False,
    )
    if accelerator.is_main_process:
        logger.info(f"loaded weight from: {os.path.join(stage1_ckpt_dir, f'imageproj.pth')}")
    ...
    resume = True # debug 使用 False为debug模式，训练为True
    if resume and cfg.resume_from_checkpoint:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", type=str, default="./configs/train/stage2_alltrain.yaml"
    )
